...
- **생성 알고리즘: '방과 복도' (Rooms and Corridors) 방식**
    ...
    3.  **복도 연결 (Corridor Connection)**: 생성된 방들을 서로 연결하는 복도를 뚫습니다. 가장 가까운 방끼리 연결하거나, 모든 방이 최소 하나 이상의 다른 방과 연결되도록 하여 고립된 지역이 없도록 만듭니다.
    4.  **입구와 출구 배치**: 플레이어의 동선을 유도하기 위해, 시작 방에는 '윗 계단'(`🔼`)을, 가장 멀리 있는 방에는 '아랫 계단'(`🔽`)을 배치합니다.
